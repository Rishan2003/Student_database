...
import { Users, GraduationCap, Layers, FileChartColumn, Settings, Plus, FileDown, ArrowUpRight, CalendarDays, Clock3, CheckCheck, RefreshCw, LogOut, ShieldCheck, Search, Pencil, BookOpen, AlertCircle, Database } from 'lucide-react';
...
const nav = [{ id: 'students' as View, label: 'Students', icon: Users }, { id: 'courses' as View, label: 'Courses', icon: GraduationCap }, { id: 'batches' as View, label: 'Batches', icon: Layers }, { id: 'reports' as View, label: 'Reports', icon: FileChartColumn }];
...
export function StudentDesk() {
    ...
    const [filters, setFilters] = useState<Filters>({ ...EMPTY_FILTERS }), [sort, setSort] = useState('name'), [selected, setSelected] = useState(''), [editing, setEditing] = useState(false), [creating, setCreating] = useState(false);
    ...
    const openStudent = useCallback((s: Student) => { setSelected(s.id); setCreating(false); setEditing(false); }, []);
    ...
    const navigate = (v: View) => { setView(v); setSelected(''); setEditing(false); setCreating(false); };
