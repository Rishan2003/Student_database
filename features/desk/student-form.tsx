...
export function StudentForm({ data, student, onBack, onOpen, mutate }: {
    data: Snapshot;
    student?: Student;
    onBack: () => void;
    ...
    const [batch, setBatch] = useState(''), [historyType, setHistoryType] = useState('none'), [historyInstitute, setHistoryInstitute] = useState(''), [historyCourse, setHistoryCourse] = useState('');
    const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<Core>({ resolver: zodResolver(coreSchema), defaultValues: student || emptyCore(data.branches[0]?.id) });
    const values = watch();
    ...
    return <div className="editor-page"><Button variant="ghost" onClick={onBack} className="back-button"><ArrowLeft size={16}/> Back to students</Button><div className="page-heading"><div><p className="eyebrow">STUDENT PROFILE</p><h1>{student ? 'Edit student' : 'Add a student'}</h1><p>{student ? student.student_code : 'One profile for every course and every next step.'}</p></div></div>
 ...
 {!student && <><TabsContent value="courses"><section className="form-card"><h2>Enroll in a batch</h2><p className="help">You can add more courses later without creating another student profile.</p><Pick label="Initial batch (optional)" value={batch} onChange={setBatch} options={[{ value: '', label: 'Add without a batch' }, ...data.batches.filter(b => ['open', 'running', 'planned'].includes(b.status)).map(b => ({ value: b.id, label: `${b.batch_code} · ${data.courses.find(c => c.id === b.course_type_id)?.name || ''}` }))]}/></section><section className="form-card"><h2>Previous learning</h2><div className="form-grid"><Pick label="Previous course history" value={historyType} onChange={setHistoryType} options={[{ value: 'none', label: 'No previous course recorded' }, { value: 'hexas', label: 'Studied at HEXA’S' }, { value: 'external', label: 'Studied at another institute' }]}/>{historyType !== 'none' && <><Field label={historyType === 'hexas' ? 'HEXA’S campus' : 'Previous institute'} value={historyInstitute} onChange={setHistoryInstitute}/><Field label="Previous course" value={historyCourse} onChange={setHistoryCourse}/></>}</div><p className="help">Additional courses and institutions can be added from the student profile.</p></section></TabsContent>
